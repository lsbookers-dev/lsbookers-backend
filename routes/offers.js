  ...
  try {
    ...
    const offer = await prisma.offer.create({
      data: {
        ...
        description,
        type,
        specialty: specialty || null, // Spécialité optionnelle
        date: offerDate,
        location,
        country,
    ...
    // Créer des notifications pour les utilisateurs correspondants
    const users = await prisma.user.findMany({
      where: {
        role: { in: type === 'ALL' ? ['ARTIST', 'PROVIDER'] : [type] },
        profile: {
          specialties: {
            hasSome: specialty ? [specialty] : undefined, // Match avec la spécialité si spécifiée
          },
          notificationPreferences: {
            locationScope: {
              in: [
                'INTERNATIONAL', // Par défaut, tous
                // On pourrait ajouter une logique pour région/France plus tard si besoin
              ],
            },
          },
        },
    ...
    for (const user of users) {
      await prisma.notification.create({
        data: {
          userId: user.id,
          type: 'NEW_OFFER',
          message: `Nouvelle offre : ${title} par ${req.user.name || 'Un organisateur'}`,
          read: false,
          offerId: offer.id,
        },
      })
